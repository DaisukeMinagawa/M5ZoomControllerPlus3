...
void show_onair() {
  ...
  if (switch_mode == "mic") {
    M5.Lcd.setTextSize(3);
    M5.Lcd.drawString("Mic", 240, 130);
  }
  else if (switch_mode == "video") {
    M5.Lcd.setTextSize(3);
    M5.Lcd.drawString("Video", 240, 130);
  } else {
    M5.Lcd.setTextSize(3);
...
void loop() {
  ...
  if (M5.Axp.GetWarningLeve() == 1) {
    ...
    if (battery > 100) {
      ...
      bleKeyboard.setBatteryLevel((int)battery);
